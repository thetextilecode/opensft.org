---
title: 'Ecommerce & Open Source'
date: '2020-03-16T05:35:07.322Z'
image: '/assets/images/blog/post-7.jpg'
imageOriginalWidth: 600
imageOriginalHeight: 400
description: A long and clouded history exists between ecommerce and open source. Many associate the latter with low quality code and security holes. But a lot has changed over the years.
draft: true
readTime: 1
trending: true
---

A long and clouded history exists between ecommerce and open source. Many associate the latter with low quality code and security holes. But a lot has changed over the years.
